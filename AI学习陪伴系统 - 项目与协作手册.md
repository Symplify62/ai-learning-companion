...
*   **技术栈：** 
    ...
    *   **AI模型:** Google Gemini 1.5 Flash, 讯飞ASR
    *   **部署:** Docker 多阶段构建, Uvicorn
    *   **工具依赖:** yt-dlp (视频下载), ffmpeg (音频处理)

---

## 3. 当前状态与核心里程碑 (迭代一完成)

**当前状态总结：** 项目已完成迭代一设定的所有核心目标，形成了一个功能可用、流程稳定、核心输出质量良好、具备基本用户界面的MVP版本，为后续迭代打下了坚实基础。B站视频到AI笔记的核心处理流程已完全打通，并已优化数据库兼容性和部署配置。关键技术风险（如讯飞API交互、长时间任务的数据库连接稳定性、AI输出质量控制）已得到有效解决和显著改善。
...
7.  **数据库兼容性优化** (从MySQL特有类型迁移到PostgreSQL兼容的通用类型)。
8.  **Docker多阶段构建实现** (生产环境准备，分离构建和运行环境)。
9.  **应用启动与错误处理增强** (调试日志增强，异常处理改进)。
10. **关键Bug修复** (AttributeError, NameError等)。
...
*   **后端架构与技术实现相关 (技术债):**
    ...
    *   *已达成:* 核心数据库稳定性问题解决；细化状态管理实现；ASR服务解耦；数据库模型PostgreSQL兼容性优化。
*   **配置与环境:**
    *   *待改进:* 
        * 前端API Base URL依赖部署时正确设置环境变量；
        * 容器化部署需完善环境变量和卷挂载配置；
        * 自动化部署流程尚未建立。
    *   *已达成:* 
        * 前端不再硬编码API URL；
...
1.  **迭代二：初步个性化与分享准备 (Should Have)**
    ...
    *   **部署与分享：**
        *   **C1: 容器化部署完善：** 完善Docker配置（环境变量传递、日志收集、卷挂载）。
        *   **C2: 云端部署实施：** 部署到云平台（如Render、Railway或AWS），生成可访问URL并进行性能监控。
        *   **C3: 生产环境数据库确认：** 确保PostgreSQL生产环境稳定运行，并实现基本的备份策略。
    *   **前端UX优化初步：**
        *   **分栏显示：** 尝试转写稿与笔记左右分栏。
        *   **状态文本优化：** 提升友好度。
        *   **基础错误处理：** 改善错误提示与恢复机制。
...
2.  **迭代三及以后：深度优化与拓展 (Could Have & Ongoing)**
    ...
    *   **技术架构持续优化：** `orchestration.py` 拆分、任务队列评估、LLM成本优化、数据库迁移策略。
    *   **功能拓展：** 更多输入源、智能问答、个性化学习路径等。
    *   **AI能力持续提升：** Prompt迭代、ASR后处理研究。
    *   **运维与监控：** 完善云端体系、日志聚合、性能分析、自动扩展。
...
*   **Docker部署最佳实践:**
    *   **多阶段构建是核心**: 分离构建环境和运行环境提升镜像效率。
    *   **调试技巧**:
        *   使用临时调试脚本(如 `debug_imports.py`)验证导入和环境。
        *   增加 Uvicorn 日志级别(`--log-level debug`)排查启动问题。
        *   在关键Python文件开头添加 `print` 语句确认解释器读取流程。
    *   **环境变量管理**:
        *   对敏感信息使用环境变量注入，避免硬编码。
        *   确保Dockerfile中的CMD指令正确传递参数。
    *   **避免的坑**:
        *   容器内的权限问题（文件和目录访问）。
        *   忘记在容器中安装ffmpeg等系统依赖。
        *   未复制yt-dlp可执行文件到最终镜像。
...
*   **数据库兼容性与迁移经验:**
    *   **通用类型是关键**:
        *   使用SQLAlchemy通用类型(如`Text`)而非特定数据库类型(如`LONGTEXT`)。
        *   为JSON字段使用标准`JSON`类型而非方言特定类型。
    *   **连接字符串格式**:
        *   PostgreSQL: `postgresql://user:pass@host:port/dbname`
        *   MySQL/OceanBase: `mysql+pymysql://user:pass@host:port/dbname`
    *   **表创建策略**:
        *   使用FastAPI启动事件 `@app.on_event("startup")` 自动创建表。
        *   为异常处理添加详细日志，捕获表创建失败。
    *   **避免的坑**:
        *   不同数据库默认字符集和排序规则的差异。
        *   长时间运行任务的连接失效问题（已通过局部会话和连接池设置解决）。
        *   忘记安装特定数据库的驱动(如`psycopg2-binary`用于PostgreSQL)。
...
8.  **重视部署环境的差异：** 本地开发环境与容器化生产环境常有重要差异，需提前考虑并验证。
9.  **关注数据库兼容性：** 不同数据库方言间存在细微差别，需使用通用类型和接口，避免特定数据库依赖。
10. **多层次错误处理策略：** 既要为用户提供友好错误信息，也要确保系统级别有详细日志用于排查问题。
